...
export const projectRouter = createTRPCRouter({
  createProject: protectedProcedure
    ...
    uploadMeeting:protectedProcedure.input(z.object({projectId:z.string(),meetingUrl:z.string(),name:z.string()})).mutation(
      async ({ctx,input})=>{
    ...
    getTeamMembers:protectedProcedure.input(z.object({projectId:z.string()})).query(async({ctx,input})=>{
      return await ctx.db.userToProject.findMany({where:{projectId:input.projectId},include:{
        user:true
      }})
    }),
    getMycredits:protectedProcedure.query(async({ctx})=>{
      ...
      const userCredits = await ctx.db.user.findUnique({where:{
        id:ctx.user.userId!
      },select:{
        credits:true
      }})
